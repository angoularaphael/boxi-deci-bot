...
function buildProductConfig(order, matchedProduct = null) {
  ...
  const scalapay = isScalapayOrder(order);
  ...
  const payplug4xDeciplus = payplug4xPrelev
    ? resolvePayplug4xPrelevementDeciplus(productHints, order)
    : null;
  const scalapayDeciplus =
    !payplug4xPrelev && scalapay ? resolveScalapayDeciplus(productHints, order) : null;
  const orderHint = [
    order.product_name,
    order.offer,
  ...
  const deciplusLabel = payplug4xDeciplus
    ...
    : scalapayDeciplus
      ? scalapayDeciplus.deciplus_product_name
      : prestation
        ? prestation.label
  ...
  return {
    ...
    deciplus_product_search:
      ...
      scalapayDeciplus?.deciplus_product_search ||
...
function scoreCatalogTile(text, productConfig = {}) {
  ...
  if (productConfig.paiement_comptant === true) {
    ...
    if (/offre\s*promo\s*12/.test(normalized) && !/4\s*[x×]|prelevement/.test(normalized)) {
      score += 160;
    }
    if (/259/.test(raw) && !/4\s*[x×]|4 fois|64[,.]75/i.test(raw)) score += 80;
    if (Number(productConfig.amount) === 259 && /comptant\s*12/.test(normalized)) score -= 180;
  }
