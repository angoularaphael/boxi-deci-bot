...
const { requiresIbanForPlan, normalizeBillingPlan, isPayplug4xPrelevementOrder } = require('./billing-plan');
...
function normalizeAction(input) {
  ...
  if (['coach_grant', 'grant', 'coach_access_grant'].includes(raw)) {
    return 'coach_grant';
  }
  if (['coach_revoke', 'revoke', 'coach_access_revoke'].includes(raw)) {
    return 'coach_revoke';
  }
  if (['coach_revoke_coach', 'revoke_coach'].includes(raw)) {
    return 'coach_revoke_coach';
  }
  return 'sale';
}
...
function getJobId(order) {
  ...
  if (order.action === 'coach_grant') return `${order.order_id}#coach_grant`;
  if (order.action === 'coach_revoke') return `${order.order_id}#coach_revoke`;
  if (order.action === 'coach_revoke_coach') return `${order.order_id}#coach_revoke_coach`;
...
function normalizeOrder(input) {
  ...
  const offer = String(
    ...
    .toLowerCase()
    .trim();
  let gym = String(input.gym || input.salle || input.club_id || '')
    .toLowerCase()
    .replace(/\s+/g, '-');
  if ((action === 'balma_switch' || action.startsWith('coach_')) && !gym) gym = 'minimes';
  ...
  const order = {
    ...
    paiement_comptant: isPayplug4xPrelevementOrder({
      ...input,
      payment,
      billing_plan: input.billing_plan || payment.billing_plan,
      payment_plan: input.payment_plan || payment.payment_plan,
    })
      ? false
    ...
    qr_valid_from: input.qr_valid_from || input.valid_from || null,
    qr_valid_to: input.qr_valid_to || input.valid_to || null,
...
function validateOrder(order) {
  ...
  if (order.action === 'coach_grant' || order.action === 'coach_revoke' || order.action === 'coach_revoke_coach') {
    const errors = [];
    if (!order.order_id) errors.push('order_id manquant');
    if (!order.gym && !order.club_id) errors.push('salle (gym / club_id) manquante');
    if (order.action === 'coach_grant' || !order.deciplus_member_id) {
      if (!order.customer?.first_name && !order.deciplus_member_id) errors.push('prénom manquant');
      if (!order.customer?.last_name && !order.deciplus_member_id) errors.push('nom manquant');
    }
    return errors;
  }
