...
/** Job completed + fiche membre, sans erreur : pas une revue manuelle (essai / séance offerte). */
function coerceCompletedTrialLifecycle(patch = {}) {
  const status = String(patch.status || '').toLowerCase();
  if (status !== 'completed' && status !== 'success') return patch.lifecycle_state || null;
  if (patch.error_message || patch.error) return patch.lifecycle_state || null;
  if (patch.sale_id || patch.deciplus_sale_id || patch.member_id || patch.deciplus_member_id) {
    return STATES.VERIFIED;
  }
  return patch.lifecycle_state || null;
}
