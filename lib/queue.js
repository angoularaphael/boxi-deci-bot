...
/** Jobs laissés en « processing » après crash / Ctrl+C → repassent en pending. */
function requeueInterruptedJobs(maxAgeMs = 2 * 60 * 1000) {
  initQueue();
  let count = 0;
  for (const f of fs.readdirSync(QUEUE_DIR)) {
    ...
    const full = path.join(QUEUE_DIR, f);
    const data = JSON.parse(fs.readFileSync(full, 'utf8'));
    if (data.status !== STATUS.PROCESSING) continue;
    ...
    updateJob(full, {
      status: STATUS.PENDING,
      last_error: data.last_error || 'Job interrompu — reprise automatique',
    });
    count += 1;
    logWarn('Job interrompu remis en file', { job_id: jobId, order_id: data.order_id });
  }
  return count;
}
