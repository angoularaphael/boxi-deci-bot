...
function canForceRequeue(record) {
  if (!record) return false;
  if (record.deciplus_sale_id) return false;
  if (record.status === STATUS.SUCCESS) return false;
  if (record.status === STATUS.DUPLICATE) return false;
  if (record.status === STATUS.REJECTED) return false;
  return record.status === STATUS.MANUAL_REVIEW || record.status === STATUS.ERROR;
}
...
function enqueue(order) {
  ...
  if (order.force_requeue) {
    ...
    if (canForceRequeue(previous)) {
      unmarkProcessed(jobId);
