...
function saleContractMatches(contractLabel, product = {}) {
  ...
  if (isOffre29Product(product)) {
    ...
    if (/offre\s*(a|duo)\s*29|\b29[,.]?0{0,2}\s*€|\b29[,.]?99/.test(label)) return true;
    // Deciplus affiche parfois un libellé générique « CONTRAT N°… » sans « OFFRE DUO 29€ ».
    if (
      /^contrat n[°o]/i.test(label) &&
      !/\bbadge\b|essai|coaching|12\s*mois|12mois|\b259\b|44,?99|etudiant/i.test(label)
    ) {
      return true;
    }
    return false;
...
/** Abo 29 € / 259 € — pas badge, essai, coaching ni 44,99. */
function isMembershipContract(contract) {
  if (!contract || contract.isBadge) return false;
  const label = normalizeLabel(contract.label);
