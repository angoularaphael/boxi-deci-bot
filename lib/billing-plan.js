...
function paymentModeLabel(product, billingPlan, paymentPlan) {
  ...
  if (plan === 'paypal') {
    return '1ʳᵉ échéance PayPal, puis prélèvement automatique';
  }
  if (plan === 'rib' || product?.requires_iban) {
    return '1ʳᵉ échéance par carte, puis prélèvement automatique';
  }
...
function paymentTodayVsNextLabel(product, billingPlan, paymentPlan) {
  ...
  if (plan === 'paypal') {
    return {
      ...
      next: 'Prochaines échéances par prélèvement automatique',
    };
  }
