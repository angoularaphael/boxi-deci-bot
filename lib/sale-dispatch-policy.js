...
function isFreeTrialDispatch(order = {}) {
  if (/^SO-/i.test(String(order.order_id || ''))) return true;
  if (order.sale_type === 'none' && order.create_sale === false) return true;
  try {
    const { isSeanceOfferteOrder } = require('./info-compta-note');
    return isSeanceOfferteOrder(order);
  } catch {
    return false;
  }
}
...
function boutiqueSaleDispatchAllowed(order = {}) {
  ...
  if (isFreeTrialDispatch(order)) return true;
