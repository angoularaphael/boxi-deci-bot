...
function buildPaymentChannelInfoComptaNote(order = {}) {
  ...
  const { isPayplug4xPrelevementOrder, isScalapayOrder } = require('./billing-plan');

  if (isScalapayOrder(order) || plan === 'scalapay' || method === 'scalapay') {
    const { scalapayInfoComptaNote } = require('./billing-plan');
    return scalapayInfoComptaNote(order);
  }
  if (plan !== '4x') return '';

  if (isPayplug4xPrelevementOrder(order) || method === 'payplug' || method === 'cawl') {
    return '4× sans frais CB';
  }
