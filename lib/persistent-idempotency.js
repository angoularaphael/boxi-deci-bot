...
async function checkpoint(orderId, action, patch = {}) {
  const sb = requireRegistry();
  let lifecycleState = patch.lifecycle_state || null;
  try {
    const { coerceCompletedTrialLifecycle } = require('./job-lifecycle');
    lifecycleState = coerceCompletedTrialLifecycle(patch) || lifecycleState;
  } catch {
    // ignore
  }
  const params = {
